...
void rmsnorm_avx512(const float* x, const float* weight, float* out,
    ...
    // Baseline scalar version (keep as reference):
    // float inv_rms = 1.0f / std::sqrt(mean_sq + eps);

    // rsqrt14 + Newton refinement (AVX512):
    //   y0 = rsqrt14(x)
    //   y1 = y0 * (1.5 - 0.5 * x * y0 * y0)
    // This improves accuracy while staying SIMD-friendly.
    float rms_arg = mean_sq + eps;
    __m512 vrms_arg = _mm512_set1_ps(rms_arg);
    __m512 vy0 = _mm512_rsqrt14_ps(vrms_arg);

    const __m512 vhalf = _mm512_set1_ps(0.5f);
    const __m512 vthree_halves = _mm512_set1_ps(1.5f);
    __m512 vy0_sq = _mm512_mul_ps(vy0, vy0);
    __m512 vterm = _mm512_mul_ps(vrms_arg, vy0_sq);
    __m512 vfactor = _mm512_sub_ps(vthree_halves, _mm512_mul_ps(vhalf, vterm));
    __m512 vy1 = _mm512_mul_ps(vy0, vfactor);
    float inv_rms = _mm512_cvtss_f32(vy1);
